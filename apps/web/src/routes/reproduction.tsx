			...
			<SelectContent>
				{animals.map((a) => (
					<SelectItem key={a.id} value={a.id}>
						{a.tag} — {a.species} ({(a as any).sex === "Male" ? "M" : "F"})
					</SelectItem>
				))}
			</SelectContent>
		</Select>
...
function ReproductionPage() {
	...
	const pregnancies: any[] = Array.isArray(pregnanciesQuery.data?.data)
		? (pregnanciesQuery.data as any).data
		: [];
