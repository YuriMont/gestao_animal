...
function LoginPage() {
  ...
  function handleRegister(e: React.FormEvent) {
    ...
    registerMutation.mutate({
      data: { name: regName, email: regEmail, password: regPassword, organizationId: regOrgId },
    })
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <div className="w-full max-w-md space-y-6">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-sm text-muted-foreground">Gestão Animal Inteligente</p>
        </div>
              ...
              <form onSubmit={handleLogin}>
                <CardContent className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <Input id="email" type="email" placeholder="nome@empresa.com" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} required />
                  </div>
                  <div className="space-y-1.5">
          ...
          <TabsContent value="register">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Cadastre-se com o ID da sua organização</CardDescription>
              </CardHeader>
              <form onSubmit={handleRegister}>
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input id="reg-email" type="email" placeholder="nome@empresa.com" value={regEmail} onChange={(e) => setRegEmail(e.target.value)} required />
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="reg-password">Senha</Label>
                    <Input id="reg-password" type="password" placeholder="Mínimo 8 caracteres" value={regPassword} onChange={(e) => setRegPassword(e.target.value)} required minLength={8} />
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="reg-org">ID da Organização (UUID)</Label>
                    <Input id="reg-org" placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" value={regOrgId} onChange={(e) => setRegOrgId(e.target.value)} required />
                  </div>
