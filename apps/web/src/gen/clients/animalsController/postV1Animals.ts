 ...
 * {@link /v1/animals}
 */
export async function postV1Animals(data: PostV1AnimalsMutationRequest, config: Partial<RequestConfig<PostV1AnimalsMutationRequest>> & { client?: Client } = {}) {
  const { client: request = fetch, ...requestConfig } = config

  const requestData = data

  const res = await request<PostV1AnimalsMutationResponse, ResponseErrorConfig<Error>, PostV1AnimalsMutationRequest>({ method : "POST", url : getPostV1AnimalsUrl().url.toString(), data : requestData, ... requestConfig })
  return res.data
}