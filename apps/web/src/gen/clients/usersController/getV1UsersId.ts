 ...
 * {@link /v1/users/:id}
 */
export async function getV1UsersId(id: GetV1UsersIdPathParams["id"], config: Partial<RequestConfig> & { client?: Client } = {}) {
  const { client: request = fetch, ...requestConfig } = config



  const res = await request<GetV1UsersIdQueryResponse, ResponseErrorConfig<Error>, unknown>({ method : "GET", url : getGetV1UsersIdUrl(id).url.toString(), ... requestConfig })
  return res.data
}