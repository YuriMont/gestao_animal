...
export const reproductionController = {
  async createEstrus(request: FastifyRequest, reply: FastifyReply) {
    ...
    const repository = new PrismaReproductionRepository(PrismaService.getInstance());
  ...
  async createPregnancy(request: FastifyRequest, reply: FastifyReply) {
    ...
    try {
      ...
      return reply.status(201).send({ message: 'Pregnancy recorded', pregnancy });
    } catch (error: any) {
      return reply.status(400).send({ error: 'Bad Request', message: error.message });
    }
  },
  ...
  async getPregnancies(request: FastifyRequest, reply: FastifyReply) {
    const tenantId = request.tenantId;
    const repository = new PrismaReproductionRepository(PrismaService.getInstance());
    const pregnancies = await repository.findPregnanciesByOrganization(tenantId!);
    return reply.send(pregnancies);
  },
  ...
  async getAnimalHistory(
    ...
    const history = await repository.findReproductionHistoryByAnimal(animalId, tenantId!);
