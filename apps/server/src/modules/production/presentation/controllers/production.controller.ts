...
export const productionController = {
  async recordWeight(request: FastifyRequest, reply: FastifyReply) {
    ...
    const repository = new PrismaProductionRepository(PrismaService.getInstance());
  ...
  async recordMilk(request: FastifyRequest, reply: FastifyReply) {
    ...
    try {
      ...
      return reply.status(201).send({ message: 'Milk production recorded', production });
    } catch (error: any) {
      return reply.status(400).send({ error: 'Bad Request', message: error.message });
    }
  },

  async getMetrics(request: FastifyRequest<{ Params: { animalId: string } }>, reply: FastifyReply) {
