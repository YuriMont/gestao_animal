...
import type { AnimalFilters, IAnimalRepository, PaginatedAnimals } from '@src/modules/core/domain/repositories/animal.repository';
...
export class PrismaAnimalRepository implements IAnimalRepository {
  ...
  async findById(id: string, organizationId: string): Promise<Animal | null> {
    const animal = await this.prisma.animal.findFirst({ where: { id, organizationId } });
    return animal ? toEntity(animal) : null;
  }

  async findByTag(tag: string, organizationId: string): Promise<Animal | null> {
    const animal = await this.prisma.animal.findFirst({ where: { tag, organizationId } });
    return animal ? toEntity(animal) : null;
  }

  async listByOrganization(organizationId: string, filters: AnimalFilters = {}): Promise<PaginatedAnimals> {
    ...
    const [animals, total] = await this.prisma.$transaction([
      this.prisma.animal.findMany({ where, skip, take: limit, orderBy: { createdAt: 'desc' } }),
