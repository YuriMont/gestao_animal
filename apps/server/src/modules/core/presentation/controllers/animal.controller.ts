...
import type { CreateAnimalDTO, ListAnimalsQueryDTO, UpdateAnimalDTO } from '@src/modules/core/presentation/dtos/animal.dto';
...
export const animalController = {
  ...
  async list(request: FastifyRequest<{ Querystring: ListAnimalsQueryDTO }>, reply: FastifyReply) {
  ...
  async update(request: FastifyRequest<{ Params: { id: string }; Body: UpdateAnimalDTO }>, reply: FastifyReply) {
    const useCase = new UpdateAnimalUseCase(getRepo());
    const animal = await useCase.execute(request.params.id, request.tenantId!, request.body);
    return reply.send({ ...animal.props, id: animal.id });
  },
