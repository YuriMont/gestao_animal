...
import type { CreateUserDTO, ListUsersQueryDTO, UpdateUserDTO } from '@src/modules/core/presentation/dtos/user.dto';
...
function safeUser(user: any) {
  return { id: user.id, email: user.props.email, name: user.props.name, role: user.props.role, organizationId: user.props.organizationId };
}
...
export const userController = {
  ...
  async list(request: FastifyRequest<{ Querystring: ListUsersQueryDTO }>, reply: FastifyReply) {
  ...
  async update(request: FastifyRequest<{ Params: { id: string }; Body: UpdateUserDTO }>, reply: FastifyReply) {
    ...
    const user = await useCase.execute(request.params.id, request.tenantId!, request.body);
    return reply.send(safeUser(user));
  },
